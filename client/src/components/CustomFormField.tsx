    ...
    switch (type) {
      case 'textarea':
        return (
          <Textarea
            ...
            {...field}
            rows={3}
            className={`border-none bg-primary-50 focus:bg-primary-100 p-4 ${inputClassName}`}
          />
        );
      case 'desc':
      ...
      case 'select':
        return (
          ...
          >
            <SelectTrigger
              className={`w-full border-none bg-primary-50 hover:bg-primary-100 focus:bg-primary-100 p-4 ${inputClassName}`}
            >
              <SelectValue placeholder={placeholder} />
            </SelectTrigger>
            <SelectContent className='w-full bg-primary-50 border-primary-200 shadow'>
              {options?.map((option) => (
                <SelectItem
                  key={option.value}
                  value={option.value}
                  className={`cursor-pointer hover:bg-primary-100 focus:bg-primary-200`}
                >
                  {option.label}
                </SelectItem>
              ))}
      ...
      case 'file':
        ...
        return (
          <FilePond
            className={`bg-primary-50 hover:bg-primary-100 ${inputClassName}`}
            files={
              // Only pass files if it's a File object
      ...
      default:
        return (
          <Input
            ...
            placeholder={placeholder}
            {...field}
            className={`border-none bg-primary-50 focus:bg-primary-100 transition-colors p-4 ${inputClassName}`}
            disabled={disabled}
          />
        );
    }
